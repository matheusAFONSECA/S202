...
cloud_config = {"secure_connect_bundle": "secure-connect-s202-prova.zip"}

# This token JSON file is autogenerated when you download your token,
# if yours is different update the file name below
with open("s202_prova-token.json") as f:
    secrets = json.load(f)
...
cassandra_session.set_keyspace("loja")  # Change to your keyspace
...
def questao_1_a(users):
    ...
    result = cassandra_session.execute(query_count_users).one()
...
def questao_1_b(products):
    ...
    query_sum_cost = "SELECT SUM(custo * quantidade) AS total_custo FROM products"
    result = cassandra_session.execute(query_sum_cost).one()["total_custo"]
...
# Questão 2
def questao_2(state):
    query = "SELECT * FROM users WHERE estado = %s ALLOW FILTERING;"
    rows = cassandra_session.execute(query, (state,))
    
    for row in rows:
        redis_conn.set(f"user:{row['id']}", json.dumps(row))
...
def questao_3(user_id):
    ...
    query = "SELECT * FROM products WHERE categoria IN %s ALLOW FILTERING;"
    rows = cassandra_session.execute(query, (tuple(interests),))
...
def test_questao_5():
    ...
    sales = [
        {"usuario": "bira@g.com", "hora": date_time.strftime("%H:%M"), "valor": 1462.78}
    ]
