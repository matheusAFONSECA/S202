...
redis_conn = redis.Redis(
    host="localhost",
    port=6379,  # use your Redis host and port
...
def questao_2(interests):
    for entry in interests:
        user_id = entry["usuario"]

        interests_data = {
            k: v for interest in entry["interesses"] for k, v in interest.items()
        }

        redis_conn.zadd(f"user:{user_id}:interests", interests_data)

    return [
        redis_conn.zrange(f"user:{entry['usuario']}:interests", 0, -1, withscores=True)
        for entry in interests
    ]
...
# Questão 3
def questao_3(posts):
    current_time = datetime.strptime("2024-06-10 20:00:00", "%Y-%m-%d %H:%M:%S")
    ...
    return [
        redis_conn.hgetall(f"post:{post['id']}")
        for post in posts
        if redis_conn.exists(f"post:{post['id']}")
    ]
...
def test_questao_3():
    posts = [
        ...
        {
            "id": "348",
            "autor": "gastro_pub@g.com",
            "data_hora": "2024-06-10 19:55:13",
        ...
        {
            "id": "349",
            "autor": "make_with_tina@g.com",
            "data_hora": "2024-06-10 19:56:44",
        ...
        {
            "id": "350",
            "autor": "samarantes@g.com",
            "data_hora": "2024-06-10 19:56:48",
        ...
        {
            "id": "352",
            "autor": "meme_e_cia@g.com",
            "data_hora": "2024-06-10 19:58:33",
...
def questao_4(user_id):
    ...
    interest_scores = {k: v for k, v in interests}

    posts = []
    for key in redis_conn.scan_iter("post:*"):
        post = redis_conn.hgetall(key)
        score = sum(
            interest_scores.get(kw, 0) for kw in post["palavras_chave"].split(", ")
        )
        posts.append((post["conteudo"], score))

    sorted_posts = [post[0] for post in sorted(posts, key=lambda x: x[1], reverse=True)]
    return sorted_posts
...
def questao_5(user_views, user_id):
    for view in user_views:
        user_id = view["usuario"]
        redis_conn.sadd(f"user:{user_id}:viewed", *view["visualizado"])  # Adicionando *
    viewed_posts = redis_conn.smembers(f"user:{user_id}:viewed")
    all_posts = [redis_conn.hgetall(key) for key in redis_conn.keys("post:*")]

    not_viewed_posts = [
        post["conteudo"] for post in all_posts if post["id"] not in viewed_posts
    ]
...
def test_questao_5():
    user_id = 3
    user_views = [
        {"usuario": 1, "visualizado": ["345", "350", "353"]},  # IDs como strings
    ...
    output = [
        "Aprenda uma receita rápida de onion rings super crocantes.",  # Corrigido aqui
        "A dica de hoje envolve os novos delineadores da linha Rare Beauty",
        "Eu quando acho a chuteira que perdi na última pelada...",
    ]
